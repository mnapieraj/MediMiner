...
Actions.prototype.handleError = function() {
	if (this.getURLParameter('error') != null) {
		if (this.getURLParameter('error') == 'import-error') {
			this
					.showError('This file format is not supported. Try to import correct CSV, XLS or ARFF file.');
		}
		return true;
	} else {
		return false;
			...
			function(data) {
				$.each(data, function(i, item) {
					$('#data-table thead').append(
							'<th title="' + item.m_Name + '"><div>'
									+ item.m_Name + '</div></th>');
					$('#data-table tfoot').append(
							'<th title="' + item.m_Name + '"><div>'
									+ item.m_Name + '</div></th>');
				});
				actions.instances.addOptionsToHeaders();
				var headersNo = actions.instances.table.find("thead")
						.find("th").length;
				$("#selectAttrDialog").find("#attrNo").attr("max",
						headersNo - 1).val(headersNo - 1);
...
Actions.prototype.buildClassifier = function(classifier) {
	...
	$.ajax(
			{
				url : 'rest?action=build&classifier=' + classifier + '&option='
						+ option,
				context : document.body
			}).done(function(data) {
...
Actions.prototype.classify = function(classifier) {
	...
	actions.showError("Classification for " + classifier
			+ " is not suported yet!");
